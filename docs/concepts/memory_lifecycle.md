...
Long-term memories aren't immutable records — they're part of the
...
embedding, and indexing — the same stages a fresh memory walks through.
...
For the full workflow — including the `PATCH /v1/long-term-memory/{id}`
endpoint, the `edit_long_term_memory` MCP tool, the editable field
reference, error handling, and worked use cases — see the [**Memory

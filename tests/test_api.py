...
class TestMemoryEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_search_long_term_memory_filters_malformed_records(self, client):
        from datetime import UTC, datetime
        ...
        with patch(
            ...
            mock_search.return_value = MemoryRecordResults(
                memories=[malformed, valid], total=2, next_offset=None
            )

            resp = await client.post(
                "/v1/long-term-memory/search",
                json={"text": "", "limit": 10, "recency_boost": False},
            )
            assert resp.status_code == 200
            data = resp.json()
            assert data["total"] == 1
            assert len(data["memories"]) == 1
            assert data["memories"][0]["id"] == "valid-1"
            assert data["memories"][0]["text"] == "Valid memory"

    @pytest.mark.asyncio
    async def test_search_long_term_memory_filters_unicode_placeholder_records(
        self, client
    ):
        from datetime import UTC, datetime
        ...
        invisible = "\u2060"  # WORD JOINER (visually blank)
        malformed = MemoryRecordResult(
            id=invisible,
            text=invisible,
            dist=0.0,
        ...
        with patch(
            ...
            assert data["total"] == 1
            assert len(data["memories"]) == 1
            assert data["memories"][0]["id"] == "valid-unicode-1"
            assert data["memories"][0]["text"] == "Valid memory"

    @pytest.mark.asyncio
    async def test_search_long_term_memory_filters_combining_mark_only_records(
        self, client
    ):
        from datetime import UTC, datetime
        ...
        combining_mark_only = "\u0301"  # COMBINING ACUTE ACCENT (visually blank)
        malformed = MemoryRecordResult(
            id=combining_mark_only,
            text=combining_mark_only,
            dist=0.0,
            created_at=now,
            updated_at=now,
            last_accessed=now,
        ...
        with patch(
            ...
            assert data["total"] == 1
            assert len(data["memories"]) == 1
            assert data["memories"][0]["id"] == "valid-combining-1"
            assert data["memories"][0]["text"] == "Visible memory"

    @pytest.mark.asyncio
    async def test_search_long_term_memory_fallback_filters_malformed_records(
        self, client
    ):
        from datetime import UTC, datetime
        ...
        with patch(
            ...
            # First call (strict filters) returns nothing to trigger soft fallback.
            # Second call (relaxed fallback) returns a malformed and a valid record.
            mock_search.side_effect = [
                MemoryRecordResults(memories=[], total=0, next_offset=None),
                MemoryRecordResults(
                    memories=[malformed, valid], total=2, next_offset=None
                ),
            ]
            ...
            assert data["total"] == 1
            assert len(data["memories"]) == 1
            assert data["memories"][0]["id"] == "valid-fallback-1"
            assert data["memories"][0]["text"] == "Valid fallback memory"

    @pytest.mark.asyncio
    async def test_create_long_term_memory_rejects_placeholder_only_text(self, client):
        payload = {
            "memories": [
                {
                    "id": "test-placeholder-text-1",
                    "text": "\u2060",  # WORD JOINER (visually blank)
                    "memory_type": "semantic",
                }
            ]
        }
        ...
        assert "placeholder-only text" in str(detail)

    @pytest.mark.asyncio
    async def test_create_long_term_memory_rejects_placeholder_only_id(self, client):
        payload = {
            "memories": [
                {
                    "id": "\u2060",  # WORD JOINER (visually blank)
                    "text": "Valid text",
                    "memory_type": "semantic",
                }
            ]
        ...
        assert response.status_code == 422
        detail = response.json().get("detail")
        assert "placeholder-only id" in str(detail)

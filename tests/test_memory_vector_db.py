...
class TestMemoryVectorDatabase:
    ...
    def test_data_to_memory_result_supports_id_and_content_aliases(self):
        """Compatibility: accept id/content aliases from legacy producers."""
        mock_index = MagicMock()
        mock_embeddings = MockEmbeddings()
        db = RedisVLMemoryVectorDatabase(mock_index, mock_embeddings)
        ...
        result = db._data_to_memory_result(fields, score=0.15)

        assert result is not None
        assert result.id == "legacy-123"
        assert result.text == "Legacy text field"
        assert result.memory_type == "message"
        assert result.dist == 0.15
    ...
    def test_data_to_memory_result_returns_none_for_missing_required_fields(self):
        """Records missing id/text should be skipped, not surfaced with placeholders."""
        mock_index = MagicMock()
        mock_embeddings = MockEmbeddings()
        db = RedisVLMemoryVectorDatabase(mock_index, mock_embeddings)
        ...
        assert db._data_to_memory_result(missing_id, score=0.1) is None
        assert db._data_to_memory_result(missing_text, score=0.1) is None

    def test_data_to_memory_result_returns_none_for_unicode_placeholder_fields(self):
        """Unicode format/separator placeholders should be treated as empty."""
        mock_index = MagicMock()
        mock_embeddings = MockEmbeddings()
        db = RedisVLMemoryVectorDatabase(mock_index, mock_embeddings)
        ...
        assert db._data_to_memory_result(malformed, score=0.1) is None

    @pytest.mark.asyncio
    async def test_list_memories_skips_malformed_records(self):
        """Filter-only listing should not return records with missing required fields."""
        mock_index = MagicMock()
        mock_index.exists = AsyncMock(return_value=True)
        ...
        result = await db.list_memories(limit=10, offset=0)

        assert result.total == 1
        assert len(result.memories) == 1
        assert result.memories[0].id == "valid-1"
        assert result.memories[0].text == "Valid memory"

    @pytest.mark.asyncio
    async def test_search_memories_skips_malformed_records(self):
        """Semantic search should skip malformed rows and keep valid ones."""
        mock_index = MagicMock()
        mock_index.exists = AsyncMock(return_value=True)
        mock_index.query = AsyncMock(
            return_value=[
        ...
        result = await db.search_memories(query="test", limit=10, offset=0)

        assert result.total == 1
        assert len(result.memories) == 1
        assert result.memories[0].id == "valid-2"
        assert result.memories[0].text == "Valid search memory"
